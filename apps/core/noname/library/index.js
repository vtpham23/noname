...
class Library {
  ...
  yingbian = {
              ...
              async (event) => {
                ...
                event.send = (player, card, source, targets, id, id2, yingbianZhuzhanAI, skillState) => {
                  ...
                  if (targets && targets.length) {
                    str += `đối với${get.translation(targets)}`;
                  }
                  str += `đã sử dụng ${get.translation(card)}，bạn có muốn bỏ 1 lá bài ${get.translation(type)} trợ chiến không？`;
                  player.chooseCard({
    ...
    effect: /* @__PURE__ */ new Map([
      ...
      [
        ...
        (event, trigger, player) => {
          ...
          trigger.baseDamage++;
          game.log(event.card, "số sát thương +1");
        }
      ],
      [
        "draw",
      ...
      [
        ...
        (event, trigger, player) => {
          ...
          game.log(event.card, "không thể hưởng ứng");
        }
      ],
      [
        "all",
        (event, trigger, player) => {
          ...
          game.log(event.card, "thực hiện tất cả");
        }
      ]
    ]),
    prompt: /* @__PURE__ */ new Map([
      ...
      ["damage", "sát thương+1"],
      ["draw", "bốc 1 lá bài"],
      ["gain", "thu lấy lá bài hưởng ứng"],
      ["hit", "lá bài này không thể bị hưởng ứng"],
      ["all", "thực hiện tất cả bất kể điều kiện"]
    ])
  };
  /**
   * Stratagem buff
  ...
  stratagemBuff = {
    ...
    effect: /* @__PURE__ */ new Map([
      [
        ...
        (event, option) => {
          ...
          game.log(event.player, "cường hoá được kích hoạt");
          game.log(
            event.card,
            "抵消所需要的",
    ...
    prompt: /* @__PURE__ */ new Map([
      ...
      ["juedou", () => "Khi gây sát thương lên mục tiêu của lá bài này, sát thương +1."],
      ["huogong", () => "Tạo thành sát thương +1。"],
      ["tao", () => "Phục hồi thể lực +1。"]
    ])
  };
  /**
  ...
  configMenu = {
    general: {
      ...
      config: {
        low_performance: {
          name: "Chế độ mượt",
          init: false,
          intro: "Giảm bớt một số hiệu ứng trong game và tăng tốc độ game.",
          onclick(bool) {
        ...
        compatible: {
          name: "Chế độ tương thích",
          init: true,
          intro: "Tính năng này cung cấp các triển khai cho một số API sắp bị loại bỏ. Bạn nên bật tính năng này khi sử dụng các tiện ích mở rộng cũ hơn. <br/>Lưu ý: Các API này sẽ bị xóa trong phiên bản tiếp theo. Tác giả tiện ích mở rộng nên tắt tùy chọn này để đảm bảo tính tương thích. (Cần thiết lập lại sau khi khởi động lại)",
          restart: true
        },
        ignore_error: {
          name: "Bỏ qua thông báo lỗi",
          init: false,
          intro: "Lỗi sẽ không được báo cáo qua cửa sổ bật lên. <br/>Lưu ý: Tùy chọn này chỉ đảm bảo rằng một số lỗi không ảnh hưởng đến trò chơi sẽ không gây ra hiện tượng đóng băng trò chơi, và không đảm bảo quá trình xử lý bình thường sau khi lỗi được báo cáo. Đối với các lỗi ảnh hưởng đến hoạt động của trò chơi, vui lòng thông báo cho tác giả tiện ích mở rộng để được điều chỉnh."
        },
        confirm_exit: {
          name: "Xác nhận thoát",
          init: false,
          intro: "Một hộp thoại xác nhận sẽ hiện lên trước khi bạn thoát khỏi trò chơi."
        },
        show_splash: {
          name: "Màn khởi động",
          intro: "Trước khi trò chơi bắt đầu, bạn sẽ vào màn hình chọn chế độ.",
          init: "init",
          item: {
            off: "Đóng",
            init: "Lần khởi động đầu tiên",
            always: "Luôn mở"
          }
        },
        game_speed: {
          name: "Tốc độ trò chơi",
          init: "mid",
          item: {
            vslow: "Rất chậm",
            slow: "Chậm",
          ...
          intro: "Tốc độ giữa các thao tác"
        },
        sync_speed: {
          name: "Giới hạn tốc độ kết toán",
          intro: "Tùy chọn này ngăn hành động mới được thực hiện trước khi kết toán hành động cũ. Nên kích hoạt tính năng này khi game bị giật lag.",
          init: true
        },
        keep_awake: {
          name: "Màn hình luôn bật",
          init: false,
          unfrequent: true,
          intro: "Ngăn màn hình tự động tắt",
          onclick(bool) {
        ...
        mount_combine: {
          name: "Hợp nhất trang bị",
          init: false,
          unfrequent: true,
          intro: "<li>Hợp nhất trang bị vào một vị trí duy nhất (có hiệu lực sau khi khởi động lại).",
          restart: true
        },
        auto_confirm: {
          name: "Tự động xác nhận",
          init: true,
          unfrequent: true,
          intro: "Nếu chỉ có duy nhất 1 mục tiêu, không cần bấm xác nhận mục tiêu"
        },
        skip_shan: {
          name: "Tự cancel khi không có thiểm",
          init: false,
          unfrequent: true,
          intro: "Khi bạn cần sử dụng hoặc đả xuất Thiểm, nếu không có thiểm, tự động bỏ qua bước này"
        },
        unauto_choose: {
          name: "Chọn bài thuận thủ, quá hà",
          init: false,
          unfrequent: true,
          intro: "Bạn luôn tự lựa chọn lá bài để thuận thủ, quá hà"
        },
        wuxie_self: {
          name: "Không vô giải bản thân",
          init: true,
          unfrequent: true,
          intro: "Khi bạn sử dụng bài lên một mục tiêu duy nhất, không hỏi vô giải bản thân"
        },
        tao_enemy: {
          name: "Không dùng Đào lên đối thủ",
          init: false,
          intro: "Trong các chế độ mà cả hai phe đều được xác định rõ ràng (như đấu tay đôi), khi nhân vật đối phương sắp chết, không hỏi xin Đào bạn",
          unfrequent: true
        },
        enable_drag: {
          name: "Cho phép kéo thả",
          init: true,
          intro: "Nhấn giữ thẻ và kéo đến vị trí mong muốn.",
          unfrequent: true
        },
        enable_dragline: {
          name: "Kéo đường chỉ báo",
          init: true,
          unfrequent: true,
          intro: "Việc xuất hiện đường chấm chấm khi kéo chuột có thể làm chậm trò chơi."
        },
        ...
        touchscreen: {
          ...
          restart: true,
          unfrequent: true,
          intro: "Kích hoạt tính năng này giúp các thiết bị màn hình cảm ứng phản hồi nhanh hơn, nhưng sẽ vô hiệu hóa thao tác chuột.",
          onclick(bool) {
        ...
        swipe: {
          ...
          init: true,
          unfrequent: true,
          intro: "Vuốt theo một trong bốn hướng trong khu vực không cuộn để thực hiện hành động tương ứng."
        },
        swipe_down: {
          name: "Vuốt xuống",
          init: "menu",
          unfrequent: true,
          intro: "Lựa chọn khi vuốt xuống",
          item: {
            system: "Hiển thị nút",
            menu: "Mở menu",
            pause: "Tạm dừng",
        ...
        round_menu_func: {
          name: "Nút cảm ứng",
          intro: "Lựa chọn khi bấm nút tròn trên màn hình",
          init: "system",
          unfrequent: true,
          item: {
            system: "Hiển thị nút",
        ...
        enable_vibrate: {
          name: "Chế độ rung",
          intro: "Rung khi lượt chơi bắt đầu",
          init: false
        },
        right_click: {
          name: "Thao tác chuột phải",
          init: "pause",
          intro: "Lựa chọn khi nhấp chuột phải vào vùng trống",
          unfrequent: true,
          item: {
            pause: "Tạm dừng",
            shortcut: "Công cụ",
        ...
        longpress_info: {
          name: "Nhấn giữ để hiển thị menu",
          init: true,
          unfrequent: true,
          restart: true,
          intro: "Nhấn giữ để hiển thị menu"
        },
        right_info: {
          name: "Nhấp chuột phải để hiển thị menu",
          init: true,
          unfrequent: true,
          restart: true,
          intro: "Nhấp chuột phải để mở menu."
        },
        hover_all: {
          name: "Thông tin hiển thị khi rê chuột",
          init: true,
          unfrequent: true,
          restart: true,
          intro: "Menu hiện lên khi di chuột"
        },
        hover_handcard: {
          name: "Thông tin hiện lên khi di chuột lên bài trên tay",
          init: true,
          unfrequent: true,
          intro: "hông tin hiện lên khi di chuột lên bài trên tay"
        },
        hoveration: {
          name: "Thời gian hiển thị menu bật lên khi di chuột",
          unfrequent: true,
          intro: "Khoảng thời gian giữa lúc di chuột qua mục tiêu và lúc menu bật lên.",
          init: "1000",
          item: {
            500: "0.5 giây",
            700: "0.7 giây",
        ...
        doubleclick_intro: {
          name: "Nhấp đúp để hiển thị thông tin chung",
          init: true,
          unfrequent: true,
          intro: "Nhấp đúp vào ảnh chân dung của một vị tướng sẽ hiển thị thẻ thông tin cá nhân của họ."
        },
        choose_all_button: {
          name: "Kích hoạt các nút Chọn tất cả/Đảo ngược lựa chọn",
          init: true,
          unfrequent: true,
          intro: "Cung cấp tùy chọn chọn tất cả/bỏ chọn khi chọn một số lượng lớn thẻ.<br><br>Một số kỹ năng có thể trở nên không hiệu quả do những hạn chế đang hoạt động hoặc các tình huống lựa chọn phức tạp."
        },
        clear_FavoriteCharacter: {
          name: "Xoá tướng yêu thích",
          clear: true,
          unfrequent: true,
          onclick() {
            if (this.innerHTML == "<span>Xác nhận xoá</span>") {
              game.saveConfig("favouriteCharacter", [], true);
              alert("Đã xoá các tướng yêu thích");
            } else {
              this.innerHTML = "<span>Xác nhận xoá</span>";
              var that = this;
              setTimeout(function() {
                that.innerHTML = "<span>Xoá các tướng yêu thích</span>";
              }, 1e3);
            }
          }
        },
        clear_BanCharacter: {
          name: "Xoá các tướng bị cấm",
          clear: true,
          unfrequent: true,
          onclick() {
            if (this.innerHTML == "<span>Xác nhận xoá</span>") {
              if (confirm("Nhấp OK để xóa tất cả các tướng bị cấm trong tất cả các chế độ; nếu không, hãy xóa các tướng bị cấm trong chế độ hiện tại.")) {
                lib.config.all.mode.forEach((mode) => game.saveConfig(`${mode}_banned`, [], mode));
                alert("Đã xoá các tướng bị cấm");
                return;
              }
              game.saveConfig(`${get.mode()}_banned`, [], true);
              alert(`${lib.mode[get.mode()]?.name ?? "本"} Đã xoá các tướng bị cấm`);
            } else {
              this.innerHTML = "<span>Xác nhận xoá</span>";
              var that = this;
              setTimeout(function() {
                that.innerHTML = "<span>Đã xoá các tướng bị cấm</span>";
              }, 1e3);
            }
          }
        },
        clear_RecentCharacter: {
          name: "Xoá các tướng chơi gần đây",
          clear: true,
          unfrequent: true,
          onclick() {
            if (this.innerHTML == "<span>Xác nhận làm rõ</span>") {
              if (confirm("Nhấp OK để xóa lịch sử chọn tướng gần đây cho tất cả các chế độ; nếu không, hãy xóa lịch sử chọn tướng gần đây cho chế độ hiện tại.")) {
                lib.config.all.mode.forEach((mode) => game.saveConfig("recentCharacter", [], mode));
                alert("Lịch sử lựa chọn tướng gần đây đã được xóa trên tất cả các chế độ.！");
                return;
              }
              game.saveConfig("recentCharacter", [], true);
              alert(`${lib.mode[get.mode()]?.name ?? "本"} Lịch sử lựa chọn tướng gần đây đã được xoá！`);
            } else {
              this.innerHTML = "<span>Xác nhận xoá</span>";
              var that = this;
              setTimeout(function() {
                that.innerHTML = "<span>Xoá các tướng chơi gần đây</span>";
              }, 1e3);
            }
          }
        },
        video: {
          name: "Lưu video",
          init: "20",
          intro: "Sau mỗi trận đấu, hãy lưu lại bản ghi hình nhiều lần nhất có thể. Khi đạt đến giới hạn này, hãy xóa các bản ghi hình bắt đầu từ bản ghi cũ nhất (các bản ghi hình đã lưu trước đó sẽ không được tính).）",
          item: {
            0: "Đóng",
            5: "5 trận",
            10: "10 trận",
        ...
        video_default_play_speed: {
          name: "Tốc độ  phát lại video mặc định",
          init: "1x",
          intro: "Thiết lập tốc độ phát lại mặc định khi xem các bản ghi trò chơi.",
          item: {
            "0.25x": "0.25x",
            "0.5x": "0.5x",
            "1x": "1x",
        ...
        max_loadtime: {
          name: "Thời gian tải lâu nhất",
          intro: "Thiết lập này xác định thời gian tối đa cần thiết để trò chơi tải từ khi khởi động. Nếu quá trình tải không hoàn tất trong thời gian này, một thông báo lỗi sẽ xuất hiện. Thời gian này có thể được tăng lên nếu thiết bị chậm hoặc nếu có nhiều tiện ích mở rộng được cài đặt.",
          init: "5000",
          unfrequent: true,
          item: {
            5e3: "5 giây",
        ...
        mousewheel: {
          ...
          init: true,
          unfrequent: true,
          intro: "Kích hoạt con lăn chuột cho phép bạn cuộn tay theo chiều ngang; tuy nhiên, nên tắt chức năng này trên các thiết bị hỗ trợ cuộn ngang, chẳng hạn như máy Mac.",
          onclick(bool) {
        ...
        auto_check_update: {
          name: "Tự động kiểm tra Update trò chơi",
          intro: "Kiểm tra cập nhật khi vào game.",
          init: false,
          unfrequent: true
        },
        lucky_star: {
          name: "Chế độ ngôi sao may mắn",
          intro: "Trong các kỹ năng liên quan đến số ngẫu nhiên, kết quả tốt nhất sẽ luôn được thu được. (Điều này không áp dụng trong chế độ trực tuyến.)",
          init: false,
          unfrequent: true
        },
        dev: {
          name: "Chế độ developer",
          intro: "Nếu kích hoạt, bạn có thể điều khiển trò chơi bằng bảng điều khiển của trình duyệt và cập nhật lên phiên bản dành cho developer.",
          init: false,
          onclick(bool) {
        ...
        extension_auto_import: {
          name: "Tự động mở rộng tiện ích",
          intro: dedent`
						Nếu kích hoạt, Vô Danh Sát sẽ tự động cập nhật các tiện ích mở rộng từ thư mục extensions
						<br />
						* Tùy chọn này không hợp lệ nếu môi trường chạy của bạn không hỗ trợ các thao tác tệp.
						<br />
						Do sự khác biệt về hiệu năng trong các thao tác tệp trên các nền tảng khác nhau, việc bật tính năng này có thể làm giảm tốc độ tải.
					`,
          init: false,
          async onclick(bool) {
        ...
        fuck_sojson: {
          name: "Phát hiện các phần mở rộng mã hóa",
          init: false,
          unfrequent: true
        },
        update_link: {
        ...
        extension_source: {
          name: "Địa chỉ bản mở rộng",
          init: "GitHub Proxy",
          unfrequent: true,
          item: {},
          intro: () => `Lấy địa chỉ khi sử dụng tiện ích mở rộng trực tuyến. Địa chỉ hiện tại.：${document.createElement("br").outerHTML}${lib.config.extension_sources[lib.config.extension_source]}`
        },
        extension_create: {
          name: "Thêm để nhận địa chỉ mở rộng",
          clear: true,
          unfrequent: true,
          onclick() {
            game.prompt("Vui lòng nhập tên địa chỉ.", function(str) {
              if (str) {
                var map = lib.config.extension_sources;
                game.prompt("Xin mời nhập địa chỉ của" + str , function(str2) {
                  if (str2) {
                    delete map[str];
                    map[str] = str2;
                    ...
                    alert("Địa chỉ mở rộng đã được thêm：" + str);
                  }
                });
              }
            });
        ...
        extension_delete: {
          name: "Xoá địa chỉ mở rộng hiện tại",
          clear: true,
          unfrequent: true,
          onclick() {
            ...
            if (!bool) {
              alert("Không thể xoá địa chỉ mở rộng cuối cùng");
              return;
            }
            ...
            alert("Đã xoá địa chỉ mở rộng：" + name2);
          }
        },
    ...
    appearence: {
      ...
      config: {
        ...
        // },
        ui_zoom: {
          name: "Điều chỉnh tỷ lệ giao diện",
          intro: "Nhập một số nguyên từ 50 đến 300 làm tỷ lệ thu phóng giao diện (hệ thống sẽ tự động chuyển đổi thành tỷ lệ phần trăm thu phóng tương ứng).",
          init: "100%",
          input: true,
          restart: true,
          onblur(e) {
            ...
            if (zoom < 50 || zoom > 300) {
              alert("Vui lòng nhập một số nguyên từ 50 đến 300");
              text.innerText = originalValue;
              return;
            }
            const zoomText = `${zoom}%`;
            const confirmed = confirm(`Bạn xác nhận muốn thay đổi tỷ lệ thu phóng giao diện thành ${zoomText} không`);
            if (!confirmed) {
              text.innerText = originalValue;
              return;
            }
        ...
        image_background: {
          ...
          visualBar: function(node, item, create) {
            ...
            node.created = true;
            var fileInput = ui.create.filediv(".menubutton", "Thêm hình nền", node, function(file) {
        ...
        show_round_menu: {
          ...
          onclick(bool) {
            ...
            if (bool && ui.roundmenu) {
              ...
              alert("关闭触屏按钮后可通过手势Mở menu（默认为下划）");
            }
          }
        },
  ...
  help = {
    ...
    游戏操作: "<ul><li>长按/鼠标悬停/右键单击显示信息。<li>触屏模式中，双指点击Tạm dừng；下划显示菜单，上划Máy tự chơi。<li>键盘快捷键<br><table><tr><td>A<td>Máy tự chơi<tr><td>W<td>切换不询问无懈<tr><td>空格<td>暂停</table><li>编辑牌堆<br>在卡牌包中修改牌堆后，将自动创建一个临时牌堆，在所有模式中共用，当保存当前牌堆后，临时牌堆被清除。每个模式可设置不同的已保存牌堆，设置的牌堆优先级大于临时牌堆。</ul>",
