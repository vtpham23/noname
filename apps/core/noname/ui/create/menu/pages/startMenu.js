...
const startMenu = function(connectMenu) {
  ...
  let startButton = ui.create.div(".menubutton.round.highlight", "Go", start, function() {
      ...
      if (infoconfig) {
        ...
        if (hiddenNodes.length) {
          if (lib.config.fold_mode) {
            ...
            var morenodes = ui.create.div(".config.more", "Thêm <div>&gt;</div>", page);
        ...
        if (!connectMenu) {
          var hidemode = ui.create.div(".config.pointerspan", "<span>Ẩn phần mở rộng này</span>", page, function() {
            if (this.firstChild.innerHTML == "Ẩn chế độ này") {
              this.firstChild.innerHTML = "Chế độ này sẽ bị ẩn sau khi khởi động lại.";
              lib.config.hiddenModePack.add(mode);
              if (!lib.config.prompt_hidepack) {
                alert("Bạn có thể khôi phục các gói mở rộng bị ẩn thông qua Tùy chọn - Khác - Đặt lại nội dung ẩn.");
                game.saveConfig("prompt_hidepack", true);
              }
            } else {
              this.firstChild.innerHTML = "Ẩn chế độ này";
