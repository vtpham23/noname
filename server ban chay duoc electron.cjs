...
// CẤU HÌNH QUAN TRỌNG: Cho phép Express truy cập vào thư mục ẩn (.pnpm)
// Đây là chìa khóa để sửa lỗi 404 node_modules của pnpm
const staticOptions = {
    dotfiles: 'allow', // Cho phép truy cập thư mục có dấu chấm (như .pnpm)
    index: false
};

// 1. Ưu tiên phục vụ file từ thư mục dist
app.use(express.static(path.join(__dirname, 'dist'), staticOptions));

// 2. Phục vụ toàn bộ thư mục gốc (Để game tìm thấy node_modules và các file .ts)
app.use(express.static(__dirname, staticOptions));

// 3. SPA Fallback: Nếu không tìm thấy file, hãy trả về index.html trong dist
